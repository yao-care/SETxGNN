...
def check_dependencies() -> Tuple[bool, List[str]]:
    ...
    try:
        import importlib
        import sys
        # 暫時移除本地路徑以檢查 conda 安裝的 txgnn
        src_paths = [p for p in sys.path if 'BrTxGNN/src' in p or 'brtxgnn/src' in p.lower()]
        for p in src_paths:
            if p in sys.path:
                sys.path.remove(p)
        # 清除本地模組快取
        mods_to_remove = [k for k in list(sys.modules.keys()) if k == 'txgnn' or k.startswith('txgnn.')]
...
class TxGNNPredictor:
    ...
    def setup(self, download_if_missing: bool = True):
        ...
        # 載入 TxGNN（從 conda 安裝的套件，避免與本地 txgnn 衝突）
        import importlib
        import sys

        # 暫時移除本地 src 路徑以避免命名衝突
        src_paths = [p for p in sys.path if 'BrTxGNN/src' in p or 'brtxgnn/src' in p.lower()]
        for p in src_paths:
            sys.path.remove(p)

        # 清除已載入的本地 txgnn 模組
        mods_to_remove = [k for k in sys.modules.keys() if k == 'txgnn' or k.startswith('txgnn.')]
        for mod in mods_to_remove:
            del sys.modules[mod]

        # 匯入 conda 安裝的 TxGNN
        txgnn_lib = importlib.import_module('txgnn')
        TxData = txgnn_lib.TxData
        TxGNN = txgnn_lib.TxGNN
    ...
    def _load_node_mappings(self):
        ...
        # 確保 ID 是字串格式（使用 conda 安裝的 txgnn.utils）
        import importlib
        txgnn_utils = importlib.import_module('txgnn.utils')
        convert2str = txgnn_utils.convert2str
        df = df.copy()
